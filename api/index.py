...
# FIX: OTPCode now stored in DB instead of in-memory dict
# (in-memory dict breaks on Vercel — each request can be a different instance)
class OTPCode(Base):
...
# ================================================================
# DB INIT AT MODULE LEVEL (runs on every cold start — required for Vercel)
# ================================================================
...
# ================================================================
# ROUTES — Admin Auth (OTP now stored in DB — works on serverless)
# ================================================================
...
async def admin_login(request: AdminLoginRequest, db: Session = Depends(get_db)):
    ...
    # Store OTP in DB (replaces broken in-memory dict)
    otp_record = OTPCode(
        email=request.email,
        code=otp,
...
async def admin_verify(request: OTPVerifyRequest, db: Session = Depends(get_db)):
    # Look up OTP from DB instead of in-memory dict
...
async def register_for_event(
    ...
    db.add(new_reg)
    db.commit()
    return {"message": "Registration submitted successfully", "registration_id": registration_id, "status": "pending"}

@app.get("/api/registrations/user")
async def get_user_registrations(email: str, db: Session = Depends(get_db)):
    registrations = db.query(Registration).filter(Registration.user_email == email).all()
    result = []
